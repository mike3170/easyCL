...
import tw.com.staninfo.easyinventory.model.WirLocList;
  ...
  public class UserLoginTask extends AsyncTask<Void, Void, Boolean> {
    ...
    protected Boolean doInBackground(Void... params) {
      ...
      try {
        ...
        // delete location
        boolean deleteOK = myDao.deleteLocation();
        if (! deleteOK) {
          this.errMessage = "刪除 location 表格失敗!";
          return false;
        }

        // 線材 --------------------------------------
        final WirLocList wirLocList = Ion.with(LoginActivity.this)
           .load(MyInfo.API_URL + "wirLocList")
           .as(new TypeToken<WirLocList>() {})
           .get();

        List<WirLocation> locationList = Arrays.asList(wirLocList.data);
        boolean insertOk = myDao.insertWirLocation(locationList);

        if (! insertOk) {
          this.errMessage = "新增 wir_location 表格失敗!";
          return false;
        }

        // 螺絲鐵桶, 剪力釘, 螺帽, 外購, 組合品, 成品棧板, 餘料
        ApiResponse<List<Location>> apiResponse = Ion.with(LoginActivity.this)
            .load(MyInfo.API_URL + "screwLocList")
            .as(new TypeToken<ApiResponse<List<Location>>>() {})
            .get();

        List<Location> screwLocationList  = apiResponse.data;

        insertOk = myDao.insertScrewLocation(screwLocationList);
        if (! insertOk) {
          this.errMessage = "新增 screw_location 表格失敗!";
          return false;
        }
        ...
        this.errMessage = getString(R.string.timeout) + " hello " + e.getMessage();
        return false;
